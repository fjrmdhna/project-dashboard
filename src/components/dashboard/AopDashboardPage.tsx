...
import {
  getAopExportEndpoint,
  getAopFiltersEndpoint,
  getAopTemplateByIdEndpoint,
  getAopTemplatesEndpoint,
} from "@/config/aop-dashboards"

const AopLoadingScreen = ({ label, message }: { label: string; message: string }) => (
  <div className="relative flex min-h-screen w-full flex-col items-center justify-center overflow-hidden bg-[#030a1f] text-white">
    ...
    <div className="relative z-10 flex w-full max-w-5xl flex-col items-center gap-10 px-6 text-center">
      <div className="flex flex-col gap-3">
        <p className="text-xs uppercase tracking-[0.65em] text-white/60">{label} Dashboard</p>
...
function AopFilterEditPanel({
  ...
  filterVariant: AopDashboardConfig["filterVariant"]
            ...
            {layout === "inline" ? (
              <>
                <div className="min-h-0 flex-1 overflow-y-auto">
                  <FilterBar value={value} onChange={onChange} onReset={onReset} variant={filterVariant} endpoint={filtersEndpoint} />
                </div>
...
export function AopDashboardPage({ config }: { config: AopDashboardConfig }) {
  const filtersEndpoint = getAopFiltersEndpoint(config)
  const templatesEndpoint = getAopTemplatesEndpoint(config)
  const exportEndpoint = getAopExportEndpoint(config)
  const mapHref = `${config.basePath}/map`
  ...
  // Prefetch filter options only after site data has loaded (avoid competing with cold-start fetch).
  useEffect(() => {
    if (aopLoading) return

    const prefetch = () => {
      prefetchFilterOptions(filtersEndpoint, config.filterVariant, false)
    }

    if (typeof window !== 'undefined' && 'requestIdleCallback' in window) {
      const idleId = window.requestIdleCallback(prefetch, { timeout: 15_000 })
      return () => window.cancelIdleCallback(idleId)
    }

    const timeoutId = setTimeout(prefetch, 2_000)
    return () => clearTimeout(timeoutId)
  }, [aopLoading, filtersEndpoint, config.filterVariant])
  ...
  const handleUpdateTemplate = async () => {
    ...
    const payload = buildTemplatePayload()
    try {
      const res = await fetch(getAopTemplateByIdEndpoint(config, selectedTemplateId), {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDeleteTemplate = async () => {
    ...
    setTemplateDeleteError(null)
    try {
      const res = await fetch(getAopTemplateByIdEndpoint(config, t.id), { method: "DELETE" })
      const json = await res.json()
      if (json.status !== "success") {
  ...
  const buildExportParams = () => {
    ...
    params.set('type', config.exportPrefix)
  ...
  const handleExport = async () => {
    try {
      ...
      let filename = `${config.exportPrefix}-export.xlsx`
      ...
      setExportStatus({
        type: 'success',
        message: `${config.label} data downloaded successfully.`
      })
    } catch (error) {
    ...
    <div className="flex h-full flex-col gap-3">
      ...
      {(aopLoading || isStaleData) && (
        <p className="text-[10px] text-white/50 flex items-center gap-1">
          ...
          {aopLoading ? `Loading ${config.label} data...` : 'Updating...'}
        </p>
      )}
      {aopError && (
        ...
        <div className="space-y-0.5 text-[10px]">
          {(!aopData || aopData.length === 0) && (
            <p className="text-white/50">
              Placeholder dataset. No {config.label} data found in database.
            </p>
          )}
    ...
    <div className="min-h-screen bg-[#050B1B] text-white flex flex-col">
      <header className="bg-gradient-to-b from-[#121c3e] to-transparent px-4 pt-5 pb-4 shadow-[0_12px_32px_rgba(0,0,0,0.35)]">
        ...
        <h1 className="mt-4 text-center text-xl font-semibold tracking-wide text-white">
          {config.dashboardTitle.toUpperCase()}
        </h1>
          ...
          <div data-mobile-templates-card className="rounded-2xl border border-white/10 bg-[#111d41]/90 backdrop-blur-sm shadow-[0_10px_30px_rgba(5,11,27,0.45)] overflow-visible">
            ...
            {isMobileFilterOpen && (
              <div className="border-t border-white/10 bg-[#050B1B]/60 px-3 py-3 space-y-3">
                ...
                {aopLoading && (
                  <p className="text-[11px] text-white/50">Loading {config.label} data...</p>
                )}
                {aopError && (
                  <p className="text-[11px] text-red-400">Error: {aopError}. Using placeholder data.</p>
                )}
                {!aopLoading && !aopError && aopData && aopData.length > 0 && (
                  <p className="text-[11px] text-white/50">Live {config.label} operational metrics from database ({aopData.length} sites).</p>
                )}
                {!aopLoading && !aopError && (!aopData || aopData.length === 0) && (
                  <p className="text-[11px] text-white/50">Placeholder data shown. No {config.label} data found in database.</p>
                )}
              </div>
            )}
          </div>
  ...
  if (!hasInitialDataLoaded) {
    const loadingMessage = aopLoading 
      ? `Retrieving latest ${config.label} operational metrics from database...`
      : "Initializing dashboard..."
    return <AopLoadingScreen label={config.label} message={loadingMessage} />
  }
