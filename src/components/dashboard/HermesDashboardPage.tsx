...
export function HermesDashboardPage({ config }: { config: HermesDashboardConfig }) {
  ...
  const stableProgramReports = useMemo(
    () => (hideProgramReport ? [] : debouncedFilterValue.program_report || []),
    [hideProgramReport, debouncedFilterValue.program_report]
  )
  ...
  const { 
    ...
    ranFilterMode: config.ranFilterMode,
  })
  ...
  const filter = useMemo(
    () => (hideProgramReport ? { ...filterValue, program_report: [] } : filterValue),
    [filterValue, hideProgramReport]
  )
  ...
  const activeFilterCount = (
    ...
    (!hideProgramReport ? filter.program_report.length : 0) +
  ...
  const handleFilterChange = (newFilters: FilterValue) => {
    ...
    console.log("Filter changed:", sanitized)
    startTransition(() => {
      filterContext.setSearchTerm(sanitized.q || '')
      filterContext.setVendorFilter(sanitized.vendor_name.length > 0 ? sanitized.vendor_name.join(',') : 'all')
      if (!hideProgramReport) {
        filterContext.setProgramFilter(sanitized.program_report.length > 0 ? sanitized.program_report.join(',') : 'all')
      }
      filterContext.setCityFilter(sanitized.imp_ttp.length > 0 ? sanitized.imp_ttp.join(',') : 'all')
      filterContext.setNanoClusterFilter(sanitized.nano_cluster.length > 0 ? sanitized.nano_cluster.join(',') : 'all')
      const circleValue = sanitized.circle && sanitized.circle.length > 0 ? sanitized.circle.join(',') : 'all'
      const ranScoreValue = sanitized.ran_score && sanitized.ran_score.length > 0 ? sanitized.ran_score.join(',') : 'all'
      filterContext.setRegionFilter(circleValue)
      filterContext.setYearFilter(sanitized.year && sanitized.year.length > 0 ? sanitized.year.join(',') : 'all')
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      let filename = `${config.exportPrefix}-${exportType}-export.xlsx`
  ...
  const matrixStats = (
    <MatrixStatsCard 
      ...
      stats={hermesStats ? {
        ...
        readiness: hermesStats.readiness,
        activated: hermesStats.activated,
  ...
  const nanoClusterCard = (
    <NanoClusterCard
      rows={rows}
      aggregatedByCluster={deferredAggregated?.byNanoCluster}
      milestoneFields={config.milestoneFields}
    />
  )
  ...
  const initialLoaderMessage = !filterContext.isHydrated
    ...
    : `Retrieving latest ${config.label} data...`

  if (!filterContext.isHydrated || !hasInitialDataLoaded) {
    return <DashboardLoadingScreen label={config.label} message={initialLoaderMessage} />
  }
