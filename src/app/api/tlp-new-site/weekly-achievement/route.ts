...
import { buildWeeklyAchievementPayload } from "@/lib/tlp-weekly-achievement"
...
export async function GET(request: Request) {
  try {
    ...
    while (hasMore) {
      const { data, error } = await applyTlpDbFilters(
        supabase
          .from("site_data_tlp")
          .select(
            "ic_000010_af, rfi_accepted, program_group, program_name, project_name, wbs_status, wo_number_1, year_from_wo, site_category, twr_owner"
          ),
        filters
    ...
    const payload = buildWeeklyAchievementPayload(rows)
    ...
    return NextResponse.json(
      {
        status: "error",
        message: "Failed to load TLP weekly achievement",
        error: message,
      },
      { status: 500 }
    )
