...
import { buildProgramSiteCategoryPayload } from "@/lib/tlp-program-site-category"
...
export async function GET(request: Request) {
  try {
    ...
    while (hasMore) {
      const { data, error } = await applyTlpDbFilters(
        supabase
          .from("site_data_tlp")
          .select(
            "program_group, project_name, site_category, program_name, wbs_status, wo_number_1, year_from_wo, twr_owner"
          ),
        filters
    ...
    const payload = buildProgramSiteCategoryPayload(rows)
    ...
    return NextResponse.json(
      {
        status: "error",
        message: "Failed to load program group site category breakdown",
        error: message,
      },
      { status: 500 }
    )
